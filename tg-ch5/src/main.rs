...
unsafe impl Hal for VirtioHal {
    fn dma_alloc(pages: usize, _direction: BufferDirection) -> (PhysAddr, NonNull<u8>) {
        ...
        unsafe {
            ...
            let offset = DMA_HEAD;
            DMA_HEAD += size;
            let base_ptr = core::ptr::addr_of_mut!(DMA_HEAP.0) as *mut u8;
...
/// 动态共享物理内存页面指针（用于 shmget/shmat）和对应的页数
static mut SHARED_PAGES: [(Option<PPN<Sv39>>, usize); 16] = [(None, 0); 16];

/// 全局信号量数组（用于 semget/semop）
static mut SEMAPHORES: [isize; 16] = [0; 16];
...
extern "C" fn rust_main() -> ! {
    ...
    for i in 0..8 {
        ...
        let header_ptr = NonNull::new(addr as *mut ()).unwrap().cast();
        if let Ok(transport) = unsafe { MmioTransport::new(header_ptr) } {
            if transport.device_type() == DeviceType::Input {
                let mut input = VirtIOInput::<VirtioHal, MmioTransport>::new(transport).unwrap();
                input.ack_interrupt(); // Actually we might just poll
                unsafe {
                    for i in 0..2 {
                ...
                log::info!("Found VirtIO GPU, initializing...");
                let mut gpu = VirtIOGpu::<VirtioHal, MmioTransport>::new(transport).unwrap();
                let (w, h) = gpu.resolution().unwrap();
                log::info!("GPU resolution: {}x{}", w, h);
                if let Ok(fb) = gpu.setup_framebuffer() {
                    unsafe {
                        FB_PTR = fb.as_mut_ptr();
                        FB_LEN = fb.len();
                        FB_WIDTH = w as usize;
                        (&mut *core::ptr::addr_of_mut!(GPU_CONTEXT)).replace(gpu);
                    }
                    log::info!("VirtIO-GPU device declared: {}x{}", w, h);
                } else {
                    log::info!("Failed to setup framebuffer");
                }
            }
        }
    }
    ...
    loop {
        ...
        // Poll VirtIO input devices for key states
        unsafe {
            for i in 0..2 {
                let slot = &mut *core::ptr::addr_of_mut!(INPUT_CONTEXTS[i]);
                if let Some(input) = slot.as_mut() {
                    while let Some(event) = input.pop_pending_event() {
                        if event.event_type == 1 { // EV_KEY
                            if event.code < 256 {
                                core::ptr::write(core::ptr::addr_of_mut!(KEY_STATES[event.code as usize]), event.value == 1);
                            }
                        }
                    }
                }
                    ...
                    match tg_syscall::handle(Caller { entity: 0, flow: 0 }, id, args) {
                        ...
                        Ret::Unsupported(_) => {
                            ...
                            match id.0 {
                                ...
                                194 => {
                                    let key = args[0] % 16;
                                    let size_bytes = args[1];
                                    let num_pages = (size_bytes + (1 << Sv39::PAGE_BITS) - 1) / (1 << Sv39::PAGE_BITS);
                                    let num_pages = if num_pages == 0 { 1 } else { num_pages };
                                    
                                    unsafe {
                                        if SHARED_PAGES[key].0.is_none() {
                                            // Allocate contiguous physical pages
                                            use core::alloc::Layout;
                                            use crate::alloc;
                                            let layout = Layout::from_size_align(num_pages << Sv39::PAGE_BITS, 1 << Sv39::PAGE_BITS).unwrap();
                                            let ptr = alloc(layout);
                                            // Zero the pages
                                            core::ptr::write_bytes(ptr, 0, num_pages << Sv39::PAGE_BITS);
                                            let ppn = PPN::<Sv39>::new((ptr as usize) >> Sv39::PAGE_BITS);
                                            SHARED_PAGES[key] = (Some(ppn), num_pages);
                                        }
                                    }
                                    *ctx.a_mut(0) = key;
                                    unsafe { (*processor).make_current_suspend() };
                                ...
                                30 => {
                                    let shmid = args[0] % 16;
                                    if let (Some(base_ppn), num_pages) = unsafe { SHARED_PAGES[shmid] } {
                                        let current = unsafe { (*processor).current().unwrap() };
                                        
                                        // Pick a high enough starting virtual address.
                                        // Using 0x4000_0000 + shmid * 0x800000 ensures huge gaps between shm allocations.
                                        let vaddr = 0x4000_0000 + shmid * 0x80_0000;
                                        
                                        for i in 0..num_pages {
                                            let vaddr_page = vaddr + i * (1 << Sv39::PAGE_BITS);
                                            let vpn = VPN::<Sv39>::new(vaddr_page >> Sv39::PAGE_BITS);
                                            let ppn = PPN::<Sv39>::new(base_ppn.val() + i);
                                            current.address_space.map_extern(vpn..vpn+1, ppn, build_flags("U_WRV"));
                                        }
                                        *ctx.a_mut(0) = vaddr;
                                    } else {
                                        *ctx.a_mut(0) = -1isize as usize;
                                    }
                                    unsafe { (*processor).make_current_suspend() };
                                }
                                // semget
                                190 => {
                                    let init_val = args[2] as isize;
                                    // Determine Sem ID based on finding a free slot or just blindly using args[0] as ID
                                    // For simplicity, let's say ID is just args[0] % 16
                                    let sem_id = args[0] % 16;
                                    unsafe { SEMAPHORES[sem_id] = init_val };
                                    *ctx.a_mut(0) = sem_id;
                                    unsafe { (*processor).make_current_suspend() };
                                }
                                // semop
                                193 => {
                                    let sem_id = args[0] % 16;
                                    let op = args[1] as isize;
                                    let current_val = unsafe { SEMAPHORES[sem_id] };
                                    if current_val + op < 0 {
                                        // Block: return -2 (like pipe empty/full) to indicate retry/yield
                                        *ctx.a_mut(0) = -2isize as usize;
                                    } else {
                                        unsafe { SEMAPHORES[sem_id] += op };
                                        *ctx.a_mut(0) = 0;
                                    }
                                    unsafe { (*processor).make_current_suspend() };
                                }
                                ...
                                129 => {
                                    ...
                                    // Note: tg-task-manage doesn't expose a direct "kill by pid" easily.
                                    // For now, let's just mark it exited if it exists.
                                    // We will find the process and remove it.
                                    // Actually, we can just use `processor.remove(pid)`.
                                    // Wait, the API for PManager doesn't expose `remove` directly.
                                    // We can iterate and find it, or we rely on the parent tracking?
                                    // Simpler: Just rely on the child exiting naturally or handle it gracefully later.
                                    log::info!("kill pid {}", target_pid);
                                    *ctx.a_mut(0) = 0;
                                    unsafe { (*processor).make_current_suspend() };
                                }
                                _ => {
                                    log::info!("unsupported syscall id = {:?}", id.0);
...
mod impls {
    use crate::{
        ...
        FB_PTR, FB_LEN, GPU_CONTEXT, KEY_STATES,
    };
    ...
    impl IO for SyscallContext {
        ...
        fn write(&self, _caller: Caller, fd: usize, buf: usize, count: usize) -> isize {
            match fd {
                ...
                4 => {
                    ...
                    let fb_ptr = unsafe { FB_PTR };
                    if fb_ptr.is_null() {
                        return -1;
                    }
                    if let Some(src_ptr) = current.address_space.translate::<u8>(VAddr::new(buf), READABLE) {
                        unsafe {
                            core::ptr::copy_nonoverlapping(src_ptr.as_ptr(), fb_ptr, count.min(FB_LEN));
                            if let Some(gpu) = (&mut *core::ptr::addr_of_mut!(GPU_CONTEXT)).as_mut() {
        ...
        fn read(&self, _caller: Caller, fd: usize, buf: usize, count: usize) -> isize {
            if fd == STDIN {
                ...
                {
                    unsafe {
                        let dst = core::slice::from_raw_parts_mut(ptr.as_mut(), count.min(256));
                        for i in 0..count.min(256) {
                            let state = core::ptr::read(core::ptr::addr_of!(KEY_STATES[i]));
                            dst[i] = if state { 1 } else { 0 };
                        }
                    }
                    count as _
                } else {
