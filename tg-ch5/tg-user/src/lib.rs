...
pub fn shmget(key: usize, size: usize) -> isize {
    unsafe { syscall2(SyscallId(194), key, size) }
}

pub fn shmat(shmid: usize) -> isize {
    unsafe { syscall1(SyscallId(30), shmid) }
}

pub fn semget(key: usize, nsems: usize, semflg: usize) -> isize {
    unsafe { syscall3(SyscallId(190), key, nsems, semflg) }
}

pub fn semop(semid: usize, op: isize) -> isize {
    loop {
        let ret = unsafe { syscall2(SyscallId(193), semid, op as usize) };
        if ret == -2 {
            sched_yield();
            continue;
        } else {
...
pub fn sys_kill(pid: usize, sig: usize) -> isize {
    unsafe { syscall2(SyscallId(129), pid, sig) }
}
