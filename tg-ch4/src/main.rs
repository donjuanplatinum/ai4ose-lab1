...
#![no_main]
#![allow(static_mut_refs, unused_imports, dead_code)]
...
unsafe extern "C" fn _start() -> ! {
    const STACK_SIZE: usize = 64 * 4096; // 256 KiB
...
const MEMORY: usize = 64 << 20; // 64 MB
...
extern "C" fn rust_main() -> ! {
    ...
    // 为堆分配器准备静态元数据空间
    static mut HEAP_METADATA: [u8; 4096] = [0; 4096];
    tg_kernel_alloc::init(unsafe { HEAP_METADATA.as_mut_ptr() as _ });
    unsafe {
    ...
    for i in 0..8 {
        ...
        if let Ok(transport) = unsafe { MmioTransport::new(header_ptr) } {
            if transport.device_type() == DeviceType::Input {
                ...
                input.ack_interrupt();
                unsafe {
                    for slot in INPUT_CONTEXTS.iter_mut() {
                        if slot.is_none() {
                            *slot = Some(input);
...
extern "C" fn schedule() -> ! {
    // 开启 S 特权级时钟中断和外部中断
    // 注意：不要在这里开启 sstatus::SIE，因为调度线程在 LocalContext 中运行。
    // 如果在 S-mode 开启 SIE，任何中断都会导致 LocalContext 退出到 rust_main 并 panic。
    // 在 U-mode 下，即使 sstatus::SIE 为 0，S-mode 的中断（sie 中的位）依然能触发并进入 stvec。
    unsafe {
        sie::set_stimer();
        sie::set_sext();
        // sstatus::set_sie(); // 不要开启这个
    }
    ...
    while !unsafe { PROCESSES.get_mut().is_empty() } {
        ...
        match scause::read().cause() {
            ...
            scause::Trap::Interrupt(scause::Interrupt::SupervisorTimer) => {
                tg_sbi::set_timer(time::read() as u64 + 125000);
            }
            // ─── 外部中断 ───
...
fn kernel_space(
    ...
    // 映射 MMIO 区域
    space.map_extern(
        VPN::new(0x10000)..VPN::new(0x10009),
        PPN::new(0x10000),
        build_flags("_WRV"),
    );
    // 映射 PLIC
    space.map_extern(
        VPN::new(0x0c000)..VPN::new(0x0c202),
        PPN::new(0x0c000),
        build_flags("_WRV"),
    );
...
mod impls {
    ...
    use crate::{INPUT_CONTEXTS, GPU_CONTEXT, FB_PTR};
                            ...
                            for i in 0..2 {
                                if let Some(input) = INPUT_CONTEXTS[i].as_mut() {
                                    if let Some(event) = input.pop_pending_event() {
                                        ...
                                        if let Some(mut ptr) = PROCESSES.get_mut()
                                            .get_mut(caller.entity)
                                            .unwrap()
                                            .address_space
                                            .translate::<virtio_drivers::device::input::InputEvent>(VAddr::new(buf), WRITABLE)
                                        {
                                            *ptr.as_mut() = event;
                                            return 24;
                                        }
            ...
            match fd {
                ...
                4 => {
                        ...
                        .translate::<u8>(VAddr::new(buf), READABLE)
                    {
                        ...
                        log::error!("ptr not readable for fb write");
                        -1
                    }
                }
                _ => {
