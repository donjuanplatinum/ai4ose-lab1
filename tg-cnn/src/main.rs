...
static mut GPU_CONTEXT: Option<VirtIOGpu<VirtioHal, MmioTransport<'static>>> = None;
static mut INPUT_CONTEXTS: [Option<VirtIOInput<VirtioHal, MmioTransport<'static>>>; 2] = [None, None];
...
static mut RNG_CONTEXT: Option<spin::Mutex<virtio_rng::VirtioRng>> = None;
...
extern "C" fn rust_main() -> ! {
    ...
    for i in 0..8 {
        ...
        if let Ok(transport) = unsafe { MmioTransport::new(header_ptr, 0x1000) } {
            ...
            } else if transport.device_type() == DeviceType::EntropySource {
                log::info!("Found VirtIO-RNG at {:#x}, initializing...", addr);
                if let Ok(rng) = virtio_rng::VirtioRng::new(transport) {
                    unsafe {
                        RNG_CONTEXT = Some(spin::Mutex::new(rng));
    ...
    unsafe {
        // 0x6000 即 (3 << 13)，将 sstatus 寄存器的 FS 字段设置为 Dirty (11)
        core::arch::asm!("csrs sstatus, {}", in(reg) 0x6000);
    }
    println!("[DEBUG] Hardware FPU enabled for kernel and user space.");
        ...
        if let Some(task) = unsafe { (*processor).find_next() } {
            ...
            match scause::read().cause() {
                ...
                scause::Trap::Exception(scause::Exception::UserEnvCall) => {
                    ...
                    // if let Ret::Done(ret) = syscall_ret {
                    //     if id != Id::WRITE {
                    //         // log::info!("[DEBUG] Syscall {:?} ret={}", id, ret);
                    //     }
                    // }
...
mod impls {
    ...
    impl IO for SyscallContext {
        fn write(&self, _caller: Caller, fd: usize, buf: usize, count: usize) -> isize {
            ...
            if fd == 1 || fd == 2 || fd == STDDEBUG {
                if let Some(ptr) = current.address_space.translate::<u8>(VAddr::new(buf), READABLE) {
                    let slice = unsafe { core::slice::from_raw_parts(ptr.as_ptr(), count) };
                    // Avoid logging every character to prevent recursion, but log when count > 0
                    if count > 0 && fd == 1 {
                        // log::info!("[DEBUG] sys_write(STDOUT): count={}", count);
                    }
                    for &c in slice {
                        // if c != 0 {
                        //     log::info!("[DEBUG] sys_write(STDOUT): {:#x} ('{}')", c, c as char);
                        // }
        ...
        if fd == STDIN {
            if let Some(ptr) = current.address_space.translate::<u8>(VAddr::new(buf), WRITEABLE) {
                ...
                for _ in 0..count {
                    ...
                    if c == usize::MAX || (c == 0 && read_count == 0) {
                    ...
                    log::info!("[DEBUG] STDIN read: {:#x}", ch);

                    // 还原你最初的指针递增逻辑
                    unsafe { *ptr = ch; ptr = ptr.add(1); }
                    read_count += 1;
                }
                return read_count as _;
            } else {
		...
		log::warn!("[DEBUG] sys_open attempt: target string = '{}', length = {}", string, string.len());
                if string == "/dev/gpu" {
                ...
                if string == "/dev/random" {
                    let new_fd = current.fd_table.len();
                    current.fd_table.push(Some(Mutex::new(Fd::VirtioRng)));
                    log::info!("Opened /dev/random as fd {}", new_fd);
                    return new_fd as isize;
                }
    ...
    impl SyncMutex for SyscallContext {
        ...
        fn condvar_wait(&self, _caller: Caller, condvar_id: usize, mutex_id: usize) -> isize {
            ...
            let tid = current.tid;
            let current_proc = unsafe { (*processor).get_current_proc().unwrap() };
            let _condvar = Arc::clone(current_proc.condvar_list[condvar_id].as_ref().unwrap());
            let mutex = Arc::clone(current_proc.mutex_list[mutex_id].as_ref().unwrap());
