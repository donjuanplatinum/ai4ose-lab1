...
wad_file_t *W_AddFile (char *filename)
{
    ...
    else 
    {
        ...
        printf("[WAD] file length=%d, numlumps=%d, infotableofs=%d, dir_size=%d\n",
               wad_file->length, header.numlumps, header.infotableofs, length);
    }
    ...
    /* Debug: print first and last few lump names */
    {
        int total = numlumps;
        printf("[WAD] Total lumps: %d\n", total);
        for (int k = startlump; k < startlump + 10 && k < total; k++)
            printf("[WAD] lump[%d] = \"%.8s\" pos=%d size=%d\n", k, lumpinfo[k].name, lumpinfo[k].position, lumpinfo[k].size);
        
        // Detailed check for font lumps
        int font_found = 0;
        for (int k = (int)startlump; k < total; k++) {
            if (strncmp(lumpinfo[k].name, "STCFN", 5) == 0) {
                if (font_found < 5) printf("[DEBUG] Found font lump: %.8s\n", lumpinfo[k].name);
                font_found++;
            }
        }
        printf("[DEBUG] Total STCFN font lumps found: %d\n", font_found);

        if (total > startlump + 10) {
            printf("[WAD] ...\n");
            for (int k = total - 3; k < total; k++)
                printf("[WAD] lump[%d] = \"%.8s\" pos=%d size=%d\n", k, lumpinfo[k].name, lumpinfo[k].position, lumpinfo[k].size);
        }
    }
...
int W_CheckNumForName (char* name)
{
    ...
    // Try hash table first, fall back to linear search
    if (lumphash != NULL)
    {
        int hash;
...
void *W_CacheLumpNum(int lumpnum, int tag)
{
    ...
    else if (lump->cache != NULL)
    {
        ...
        // Safety check: verify the zone block header is valid before Z_ChangeTag.
        // The ZONEID (0x1d4a11) is stored in the 'id' field at a fixed offset
        // before the data pointer. On 64-bit: sizeof(memblock) = 48, id is at offset 32.
        // Rather than relying on internal layout, just attempt the tag change and
        // recover if it would fail.
        {
            /* memblock_t on 64-bit: size(4)+pad(4)+user(8)+tag(4)+id(4)+next(8)+prev(8) = 40 bytes */
            /* id is at offset 20, so from data ptr: data - 40 + 20 = data - 20 */
            int *possible_id = (int *)((byte *)lump->cache - 20); /* id field */
            if (*possible_id != 0x1d4a11) {
                printf("[WARN] lump[%d] (%.8s) cache=%p bad ZONEID, reloading\n",
                       lumpnum, lump->name, lump->cache);
                lump->cache = NULL;
                goto load_fresh;
            }
        }
