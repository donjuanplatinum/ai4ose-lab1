...
void HU_Init(void)
{
    ...
    // load the heads-up font (gracefully handle missing lumps for shareware WAD)
    j = HU_FONTSTART;
	...
	if (lumpnum >= 0) {
	    hu_font[i] = (patch_t *) W_CacheLumpNum(lumpnum, PU_STATIC);
	    // printf("HU_Init: Loaded %s (lump %d) at %p\n", buffer, lumpnum, hu_font[i]);
	} else {
	    hu_font[i] = NULL;
	    printf("HU_Init: Lump %s NOT FOUND\n", buffer);
	}
    }
